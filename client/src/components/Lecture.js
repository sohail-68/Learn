...
const CreateLecture = () => {
  ...
  useEffect(() => {
    if (states) {
      ...
      setVideoPreview(
        states.video ? `https://learn-rd8o.onrender.com/${states.video.replace(/\\/g, '/')}` : null
      );
    }
  }, [states]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!states) {
        ...
        await axios.post('https://learn-rd8o.onrender.com/lecture', formData, {
          headers: {
        ...
        await axios.put(`https://learn-rd8o.onrender.com/lecture/lecture/${states._id}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
